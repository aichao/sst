...
#ifndef iTrek_request_item_hpp
#define iTrek_request_item_hpp
...
/**
 */
class request_item : public command_state {
 public:
  /** This method processes the next token in the queue and adds it to the
   *  command as the request item. The command is then complete and this
  ...
  virtual boost::optional<std::string> prompt() const {
    return boost::optional<std::string>("Information desired? ");
  }
};
