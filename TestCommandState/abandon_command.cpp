...
command_state* create_abandon_command_state() {
  return new abandon_command();
}
// Define the identifier
const std::string abandon_command_state_id("abandon");
// Register
const bool registered = command_state_factory::instance().register_command_state(
    abandon_command_state_id, create_abandon_command_state);

}

boost::logic::tribool abandon_command::handle(command_input_handler* handler) const {
  // append the command "abandon" to the command data
  append_command_data(handler, "abandon");
  // clear token queue
