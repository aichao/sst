...
command_state* create_transport_command_state() {
  return new transport_command();
}
// Define the identifier
const std::string transport_command_state_id("transport");
// Register
const bool registered = command_state_factory::instance().register_command_state(
    transport_command_state_id, create_transport_command_state);

}

boost::logic::tribool transport_command::handle(command_input_handler* handler) const {
  // append the command "transport" to the command data
  append_command_data(handler, "transport");
  // clear token queue
