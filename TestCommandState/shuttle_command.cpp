...
command_state* create_shuttle_command_state() {
  return new shuttle_command();
}
// Define the identifier
const std::string shuttle_command_state_id("shuttle");
// Register
const bool registered = command_state_factory::instance().register_command_state(
    shuttle_command_state_id, create_shuttle_command_state);

}

boost::logic::tribool shuttle_command::handle(command_input_handler* handler) const {
  // append the command "shuttle" to the command data
  append_command_data(handler, "shuttle");
  // clear token queue
