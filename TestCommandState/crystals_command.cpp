...
command_state* create_crystals_command_state() {
  return new crystals_command();
}
// Define the identifier
const std::string crystals_command_state_id("crystals");
// Register
const bool registered = command_state_factory::instance().register_command_state(
    crystals_command_state_id, create_crystals_command_state);

}

boost::logic::tribool crystals_command::handle(command_input_handler* handler) const {
  // append the command "crystals" to the command data
  append_command_data(handler, "crystals");
  // clear token queue
