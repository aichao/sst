...
command_state* create_dock_command_state() {
  return new dock_command();
}
// Define the identifier
const std::string dock_command_state_id("dock");
// Register
const bool registered = command_state_factory::instance().register_command_state(
    dock_command_state_id, create_dock_command_state);

}

boost::logic::tribool dock_command::handle(command_input_handler* handler) const {
  // append the command "dock" to the command data
  append_command_data(handler, "dock");
  // clear token queue
