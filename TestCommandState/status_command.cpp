...
command_state* create_status_command_state() {
  return new status_command();
}
// Define the identifier
const std::string status_command_state_id("status");
// Register
const bool registered = command_state_factory::instance().register_command_state(
    status_command_state_id, create_status_command_state);

}

boost::logic::tribool status_command::handle(command_input_handler* handler) const {
  // append the command "status" to the command data
  append_command_data(handler, "status");
  // clear token queue
