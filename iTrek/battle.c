 ...
 *  2) i = 1: interactive mode.
 *  if SCAN is "transfer"
 *    repeatedly SCAN and ASK for amount of energy to transfer until SCAN is a number
 *    set parameter to "transfer <amount>"
 *  else if shields are damaged
 *    PRINT shields are damaged and down
 *    set parameter to "none"
 *  else if SCAN is "up"
 *    set parameter to "up"
 *  else if SCAN is "down"
 *    set parameter to "down"
 *  else
 *    ASK if user want to change shield energy.
 *    if answer is yes, 
 *      ASK for amount of energy to transfer.
 *      repeatedly SCAN and ASK for amount of energy to transfer until SCAN is a number
 *      set parameter to "transfer <amount>"
 *    else if shields are damaged
 *      PRINT shields are damaged and down
 *      set parameter to "none"
 *    else if shields are up,
 *      ASK if user wants to lower shields
 *      if answer is yes,
 *        set parameter to "down"
 *      else
 *        set parameter to "none"
 *    else if shields are down,
 *      ASK if user wants to raise shields
 *      if answer is yes,
 *        set parameter to "up"
 *      else
 *        set parameter to "none"
 *  Note: with this logic one can transfer energy to shields even if it is damaged,
 *  but one cannot raise or lower shields if they are damaged.
 */
void sheild(int i) {
	int key;
 ...
 *    end
 *  else if SCAN is EOL
 *    repeatedly PRINT number of torpedoes left, then 
 *      ASK and SCAN for number of torpedoes to fire until number is less than or equal to the available torpedoes
 *      if SCAN is alpha or a number less than or equal to zero
 *        set parameter to "none"
 *        end
 *      else if SCAN is a number greater than 3
 *        PRINT maximum of 3 torpedoes per burst
 *    set N (number of torpedoes to fire) to SCAN
 *  else if SCAN is a number less than or equal to zero
 *    set parameter to "none"
 *    end
 *  else if SCAN is a number greater than 3
 *    PRINT maximum of 3 torpedoes per burst
 ...
 *        PRINT maximum of 3 torpedoes per burst
 *    set N (number of torpedoes to fire) to SCAN
 *  else if SCAN is a number greater than number of available torpedoes
 *    repeatedly PRINT number of torpedoes left, then
 *      ASK and SCAN for number of torpedoes to fire until number is less than or equal to the available torpedoes
 *      if SCAN is alpha or a number less than or equal to zero
 *        set parameter to "none"
 ...
 *    set N (number of torpedoes to fire) to SCAN
 *  if SCAN is EOL
 *    Loop over number of torpedoes to fire
 *      Ask for sector coordinates for ith torpedo
 *      if SCAN is not a number
 *        set parameter to "none"
 *        end
 *      else
 *        set row coordinate for third torpedo to SCAN
 *      if SCAN is not a number
 *        set parameter to "none"
 *        end
 *      else
 *        set column coordinate for third torpedo to SCAN
 *      if row and column coordinates are not valid
 *        set parameter to "none"
 *        end
 *    set parameter to "<N> <y1> <x1> ... <yN> <xN>"
 *    end
 *  else if SCAN is not a number
 *    set parameter to "none"
 *    end
 *  else
 *    set row coordinate for first torpedo to SCAN
 *  if SCAN is not a number
 *    set parameter to "none"
 *    end
 *  else
 *    set column coordinate for first torpedo to SCAN
 *  if row and column coordinates are not valid
 *    set parameter to "none"
 *    end
 *  if number of torpedoes is equal to 1
 *    set parameter to "1 <y1> <x1>"
 *    end
 *  if SCAN is EOL
 *    set parameter to "<N> <y1> <x1>"
 *    end
 *  else if SCAN is not a number
 *    set parameter to "none"
 *    end
 *  else
 *    set row coordinate for second torpedo to SCAN
 *  if SCAN is not a number
 *    set parameter to "none"
 *    end
 *  else
 *    set column coordinate for second torpedo to SCAN
 *  if row and column coordinates are not valid
 *    set parameter to "none"
 *    end
 *  if number of torpedoes is equal to 2
 *    set parameter to "2 <y1> <x1> <y2> <x2>"
 *    end
 *  if SCAN is not a number
 *    set parameter to "none"
 *    end
 ...
 *    end
 *  else
 *    set parameter to "3 <y1> <x1> <y2> <x2> <y3> <x3>"
 *    end
 */
void photon(void) {
...
/** Phasers: interactive mode
 *  if SCAN is EOL,
 *    if there are no enemies in quadrant,
 *    else if there is damage in either SRS or Computer,
 *    else
 *      repeatedly ASK "manual or automatic" and SCAN 
 *
 */
void phasers(void) {
	...
	if (shldup) {
		...
		ifast = 1; // Note ifast == 1 iff shldup && ( phasers not damaged and energy > 200.0 ) otherwise ifast == 0
  ...
  // Entering here we have a current input with key, which can be EOL
	switch (automode) {
		case AUTOMATIC:
      // If current input is a string and it is "no", then set no and get the next input
			if (key == IHALPHA && isit("no")) {
				no = 1;
				key = scan();
			}
      // if current input is not a number and there are enemies, display message "Phasers locked on target. Energy available =".
			if (key != IHREAL && nenhere != 0) {
				proutn("Phasers locked on target. Energy available =");
				cramf(ifast?energy-200.0:energy,1,2);
				skip(1);
			}
      // This do-while loop repeatly prompts the user to input "units to fire=" until input is a number and value is within energy limits
      // If input is not a number, reject input and repeat prompt. If value is beyond limit, display "Enegry available= " message.
			do {
				while (key != IHREAL) {
					chew();
      ...
      // If input value is less than or equal to zero, ignore command
			if (rpow<=0) {
				/* chicken out */
				ididit = 0;
				chew();
      ...
      // Get the next input and see if it is no (note ths will usually be just EOL), if so, set no
			if ((key=scan()) == IHALPHA && isit("no")) {
				no = 1;
			}
      // This is part of command execution
			if (ifast) {
      ...
      // clear command queue
			chew();
      // The rest is all part of command execution
			energy -= rpow;
			extra = rpow;
			if (nenhere) {
				extra = 0.0;
		...
		case FORCEMAN:
      // forceman only if computer or srs or both are damaged
      // clear command queue and set key to EOL
			chew();
			key = IHEOL;
      // if computer is damaged (or both) then say computer is damaged, otherwise say srs is damaged
			if (damage[DCOMPTR]!=0)
      ...
      // then proceed to manual mode
		case MANUAL:
			rpow = 0.0;
      // loop over all enemy targets
			for (k = 1; k <= nenhere;) {
				int ii = kx[k], jj = ky[k];
				int ienm = quad[ii][jj];
        // print message of energy available if:
        // (i) first target
        // (ii) if user inputs EOL for the energy to target the first target
        // (iii) if total requested energy exceeds available in which case, we start over.
				if (msgflag) {
					proutn("Energy available= ");
        ...
        // if srs is damaged and target is far (> 2) in either x or y direction and target is 
        // a commander or a super commander, then say that target cannot be targeted.
        // clear the command queue, set key to EOL and go to next target
      ...
      // cler the command queue, the rest is all part of command execution
			energy -= rpow;
			chew();
			if (ifast) {
				energy -= 200.0;
